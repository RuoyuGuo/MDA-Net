...
class KPNV2(nn.Module):
    def __init__(self, kernel_size=[3], sep_conv=False, channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(KPNV2, self).__init__()
        self.upMode = upMode
        self.core_bias = core_bias
        self.kernel_size = kernel_size
    ...
    def forward(self, data_with_est, x, att_fea):
        ...
        #print('kernels.shape before interporlate: ', kernels.shape)
...
class KPNV3(nn.Module):
    def __init__(self, kernel_size=[3], sep_conv=False, channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(KPNV3, self).__init__()
        print('Using KPNV3.......................')
        self.upMode = upMode
        self.core_bias = core_bias
        self.kernel_size = kernel_size
    ...
    def forward(self, data_with_est, x, att_fea, att_fea2):
        ...
        kernels = self.kernels(conv3 * att_fea)
        kernels = kernels.unsqueeze(dim=0)
        ...
        conv8 = self.conv8(torch.cat([conv2, F.interpolate(conv7, scale_factor=2, mode=self.upMode)], dim=1))
        conv9 = self.conv9(torch.cat([conv1, F.interpolate(conv8, scale_factor=2, mode=self.upMode)], dim=1))
        core_img = self.core_img(conv9 * att_fea2)

        return kernels, core_img
    
    
class KPNRevision(nn.Module):
    def __init__(self, att_level, kernel_size=[3], sep_conv=False, channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(KPNRevision, self).__init__()
        print('Using KPNV3.......................')
        self.upMode = upMode
        self.core_bias = core_bias
        self.kernel_size = kernel_size
        self.att_level = att_level
...
class KernelConv(nn.Module):
    ...
    def forward(self, frames, core, white_level=1.0, rate=1):
        ...
        for index, K in enumerate(kernel):
            ...
            if not img_stack:
                ...
                for i in range(0, K):
                    for j in range(0, K):
                        ...
                        #print('img_stack shape, inner', frame_pad[..., i*rate:i*rate + height, j*rate:j*rate + width].shape)
            ...
            #print('img_stack shape, out', img_stack.shape)
